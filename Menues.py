...
class Menu:
    ...
    def __init__(self, buttons_arr, background, menu_num):

        self.buttons_arr = buttons_arr
...
def player_set():
    ...
    ship_highlights_3 = B_Ship_Highlihgts((170, 20, 60, 200), 1)
        ...
        for event in pygame.event.get():
            ...
            if keys[pygame.K_UP]:
                ...
                if selection[0] > 0:
                    ...
                    menu[selection[0]-1][0].select()
                    menu[selection[0]][0].deselect()
                    selection[0] += -1
                    screen.blit(temporary_BG, (0,0))
                    for y in menu:
                        for x in y:
